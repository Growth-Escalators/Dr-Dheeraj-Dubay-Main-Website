...
function ColumnHeading({ icon: Icon, children }: { icon: React.ElementType; children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3">
      <span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-emerald-100 bg-emerald-50 text-emerald-600 dark:border-emerald-800/60 dark:bg-emerald-900/30 dark:text-emerald-400"><Icon className="h-5 w-5" /></span>
      <div>
        <h3 className="text-lg font-bold text-emerald-900 dark:text-gray-100">{children}</h3>
...
function NavItem({ href, external, children }: { href: string; external?: boolean; children: React.ReactNode }) {
  const className = "group inline-flex items-center gap-2 text-[15px] text-gray-600 transition-colors hover:text-emerald-700 dark:text-gray-300 dark:hover:text-emerald-400";
  const inner = <><ChevronRightIcon className="h-4 w-4 shrink-0 text-emerald-500 transition-transform duration-200 group-hover:translate-x-0.5" /><span>{children}</span></>;
  return external ? <a href={href} target="_blank" rel="noopener noreferrer" className={className}>{inner}</a> : <Link href={href} className={className}>{inner}</Link>;
}

const socialLinks = [
  { name: "Facebook", href: "https://www.facebook.com/drdheerajdubay/", icon: FacebookIcon, colorClass: "text-[#1877F2]" },
  { name: "Instagram", href: "https://www.instagram.com/dheerajdubay1/", icon: InstagramIcon, colorClass: "text-[#E1306C]" },
  { name: "LinkedIn", href: "https://www.linkedin.com/in/dr-dheeraj-dubay-36399599/", icon: LinkedinIcon, colorClass: "text-[#0A66C2]" },
          ...
          <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-4 lg:gap-0 lg:divide-x lg:divide-emerald-100 dark:lg:divide-gray-700/60">
            <div className="text-center lg:pr-10">
              <Link href="/" className="mx-auto block w-44">
                <Image height={300} width={300} src="/assets/images/logofinalbg.png" alt="Dr. Dheeraj Dubay" sizes="176px" className="h-auto w-full transition-transform duration-300 hover:scale-105" />
              </Link>
              <div className="my-4 flex items-center justify-center gap-3"><span className="h-px w-12 bg-emerald-200" /><StethoscopeIcon className="h-4 w-4 text-emerald-500" /><span className="h-px w-12 bg-emerald-200" /></div>
              ...
              <div className="mt-5 inline-flex items-center gap-2 rounded-full border border-emerald-100 bg-white/70 px-4 py-2 text-sm font-medium text-gray-700 dark:border-emerald-900/50 dark:bg-gray-800/60 dark:text-gray-200"><MapPinIcon className="h-4 w-4 text-emerald-500" />Shalby Hospital, Jaipur</div>
            </div>
            ...
            <div className="lg:px-10">
              ...
              <ul className="mt-7 space-y-4">
                {PRIORITY_JAIPUR_PAGES.slice(0, 6).map((item) => <li key={item.href}><NavItem href={item.href}>{item.label}</NavItem></li>)}
              </ul>
            </div>
            ...
            <div className="lg:pl-10">
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                {socialLinks.map((social) => (
                  <a key={social.name} target="_blank" rel="noopener noreferrer" href={social.href} className="group inline-flex h-12 w-12 items-center justify-center rounded-xl border border-gray-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md dark:border-gray-700 dark:bg-gray-800">
                    <social.icon className={`h-6 w-6 ${social.colorClass} transition-transform duration-300 group-hover:scale-110`} /><span className="sr-only">{social.name}</span>
                  </a>
                ))}
              </div>
              <div className="mt-6 rounded-2xl border border-emerald-100 bg-white/70 p-5 text-sm leading-relaxed text-gray-600 shadow-sm dark:border-emerald-900/50 dark:bg-gray-800/60 dark:text-gray-300">
                Medical information on this website is educational and does not replace an in-person examination or personalised treatment plan. <Link href="/editorial-policy" className="font-semibold text-emerald-700 hover:underline dark:text-emerald-400">How we handle medical content.</Link>
              </div>
            </div>
          </div>
        </div>
